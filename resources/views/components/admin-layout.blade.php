...
<body class="h-full font-sans antialiased bg-slate-50">
    <div x-data="{ open: false }" class="min-h-screen">
            ...
            <div class="fixed inset-0 flex">
                <div x-show="open" x-transition:enter="transition ease-in-out duration-300 transform" x-transition:enter-start="-translate-x-full" x-transition:enter-end="translate-x-0" x-transition:leave="transition ease-in-out duration-300 transform" x-transition:leave-start="translate-x-0" x-transition:leave-end="-translate-x-full" class="relative mr-16 flex w-full max-w-xs flex-1">
                    <div class="absolute left-full top-0 flex w-16 justify-center pt-5">
                        <button type="button" @click="open = false" class="-m-2.5 p-2.5 text-white">
                    ...
                    <!-- Sidebar mobile -->
                    <div class="admin-sidebar flex grow flex-col gap-y-5 overflow-y-auto px-6 pb-4 ring-1 ring-white/10">
                            ...
                            <ul role="list" class="flex flex-1 flex-col gap-y-7">
                                <li>
                                    <ul role="list" class="-mx-2 space-y-1">
                                        ...
                                        <li>
                                            <a href="{{ route('admin.restaurants.index') }}" class="admin-nav-link {{ request()->routeIs('admin.restaurants.*') ? 'admin-nav-link-active' : '' }}">
                                                <svg class="h-6 w-6 shrink-0" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                        ...
                                        <li>
                                            <a href="{{ route('admin.orders.index') }}" class="admin-nav-link {{ request()->routeIs('admin.orders.*') ? 'admin-nav-link-active' : '' }}">
                                                <svg class="h-6 w-6 shrink-0" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
        ...
        <div class="hidden lg:fixed lg:inset-y-0 lg:z-50 lg:flex lg:w-72 lg:flex-col">
            <div class="admin-sidebar flex grow flex-col gap-y-5 overflow-y-auto px-6 pb-4">
                <div class="flex h-16 shrink-0 items-center">
                    <a href="{{ route('admin.dashboard') }}" class="text-xl font-bold text-white">
                        ClicknEat Admin
                    </a>
        ...
        <!-- Header mobile -->
        <div class="admin-header sticky top-0 z-40 flex items-center gap-x-6 px-4 py-4 shadow-sm sm:px-6 lg:hidden">
            <button type="button" @click="open = true" class="-m-2.5 p-2.5 text-slate-200 hover:text-white lg:hidden">
                <span class="sr-only">Ouvrir le menu</span>
                <svg class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M3.75 6.75h16.5M3.75 12h16.5m-16.5 5.25h16.5" />
                </svg>
            </button>
            <div class="flex-1 text-sm font-semibold leading-6 text-white">ClicknEat Admin</div>

            <!-- Menu utilisateur -->
            <div class="relative" x-data="{ open: false }" @click.outside="open = false" @close.stop="open = false">
                <button @click="open = !open" class="flex items-center text-sm font-medium text-slate-200 hover:text-white focus:outline-none transition duration-150 ease-in-out">
                ...
                <div x-show="open"
                     ...
                     class="admin-dropdown absolute right-0 mt-2 w-48 py-1 z-50"
                     style="display: none;">
